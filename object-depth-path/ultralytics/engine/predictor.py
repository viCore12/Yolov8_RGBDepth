            ...
            for self.batch in self.dataset:
                ...
                with profilers[0]:
                    ...
                    for i in range(im.size(0)):
                        ...
                        depth_image_np = np.random.rand(256, 256) #######################################
                        
                        # Convert the numpy depth image to a PyTorch tensor
                        depth_image_tensor = torch.from_numpy(depth_image_np)
                        depth_image_tensor = depth_image_tensor.unsqueeze(0)  # Shape: (1, 439, 640)
                        
                        rgbd_image = torch.cat((rgb_image_resized, depth_image_tensor.to(self.device)), dim=0)  # Shape: (4, 439, 640)
                        images_rgbd.append(rgbd_image)
                    im = torch.stack(images_rgbd).float()
